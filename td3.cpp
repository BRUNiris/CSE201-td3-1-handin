...
double* append_to_array(double element,
    ...
    if (current_size >= max_size) {
        array = extend_array(array, current_size, 5+max_size);
        max_size = max_size + 5;
    }
...
double* remove_from_array(double* array,
    ...
    if (abs(current_size-max_size-1) >= 5) {
...
void sort(double *targets, const int num_targets) {
    ...
    while (sorted == 1) {
        sorted = 0;
        for (int i = 0; i < num_targets-3; i=i+3) {
            if (targets[i] > targets[i+3]){
                swap(targets[i], targets[i+3]);
                swap(targets[i+1], targets[i+4]);
                swap(targets[i+2], targets[i+5]);
                sorted = 1;
            }
        }
    }
